...
% [Clean,fs,nbits]=wavread([pathname filename])';
% [filename,pathname]=uigetfile('*.wav','请选择增强语音文件：');
...
LSD=mean(sqrt(mean((log( max(CL(RangeBin,1:N),1e-8))-log( max(NO(RangeBin,1:N),1e-8) )).^2)));
