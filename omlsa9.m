function [cplot,nplot,splot,noiseplot,data_in,out]=omlsa9(fclean,fnoise,fin,fout)
% 在omlsa1的基础上改进了IMCRA算法——基于SER的IMCRA算法
% 参考论文：《Noise Spectrum Estimation with Improved Minimum Controlled Recursive Averaging based on Speech Enhancement Residue》
...
alpha_s=0.9;	% 2.2)  平滑操作的递归平均参数
Uwin=8; 	% 2.3)  局部最小搜索的分解
Vwin=15;
Bmin=1.66;
xi_0=1.67;		% 2.4)  局部最小因子
...
fnoiseID = fopen([fnoise '.wav'],'r');
fcleanID = fopen([fclean '.wav'],'r');
cplot=[];
splot=[];
nplot=[];
noiseplot=[];
for i=1:nframes
    if i==1
        ...
        initfnoise=fread(fnoiseID,22,'int16');
        initfclean=fread(fcleanID,22,'int16');
        data_in=fread(finID,nwin,'int16')/2^15;
        noise_in=fread(fnoiseID,nwin,'int16')/2^15;
        clean_in=fread(fcleanID,nwin,'int16')/2^15;
    else
        y0=fread(finID,inc,'int16')/2^15;
        n0=fread(fnoiseID,inc,'int16')/2^15;
        c0=fread(fcleanID,inc,'int16')/2^15;
        data_in=[data_in(inc+1:nwin,:) ; y0];      % 更新数据帧
        noise_in=[noise_in(inc+1:nwin,:) ; n0];
        clean_in=[clean_in(inc+1:nwin,:) ; c0];
    end
    
    if (~fnz_flag && abs(data_in(1))>zero_thres) ||  (fnz_flag && any(abs(data_in)>zero_thres))       % 新版本omlsa3
        fnz_flag=1;     
        % 1. 短时傅立叶分析
        Y=fft(win.*data_in);
        N=fft(win.*noise_in);
        ...
        if i==i_fnz     
            ...
            N1=alpha_s*N1+(1-alpha_s)*Nf;
            C1=alpha_s*C1+(1-alpha_s)*Cf;
        end
        if i<14+i_fnz     
            Smin=S;
            SMact=S;
        ...
        noiseplot=[noiseplot,lambda_d(32)];
    ...
    if i==1
        foutID = fopen([fout '.wav'],'w');      % 打开输出文件并写入第一个输出示例
        fwrite(foutID,[initfin; out(1:inc)*2^15],'int16');
    else
        fwrite(foutID,out(1:inc)*2^15,'int16');   % 写入输出文件
    end
    out=[out(inc+1:nwin); zeros(inc,1)];   % 更新输出帧
end
fwrite(foutID,out(1:nwin-inc)*2^15,'int16');  % 写入输出文件上一个输出示例
fclose(finID);    % 关闭输入文件
fclose(foutID);   % 关闭输出文件
