function [cplot,nplot,splot,noiseplot,data_in,out]=omlsa8(fclean,fnoise,fin,fout)
% 在omlsa1的基础上基于语音存在概率选择搜索窗长
% 参考论文：《一种改进型 IMCRA 非平稳噪声估计算法》
...
for i=1:nframes
    ...
    if (~fnz_flag && abs(data_in(1))>zero_thres) ||  (fnz_flag && any(abs(data_in)>zero_thres))       % 新版本omlsa3
        ...
        lambda_dav=alpha_dt.*lambda_dav+(1-alpha_dt).*Y2;
        if i<14+i_fnz     
